...
export default function DiarioTab({ turmaId }) {
    ...
    async function carregar() {
        setLoading(true);
        try {
            const data = await api.get(`/diario?turma_id=${turmaId}`);
            setEntradas(data || []);
        } catch {
            setEntradas([]);
        } finally {
    ...
    if (loading) return <p style={{ color: '#888', fontSize: '13px' }}>Carregando diário...</p>;
    ...
    return (
        <div style={{ display: 'grid', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={sectionTitle}>Diário de Classe</h3>
            ...
            {entradas.length === 0 ? (
                <div style={emptyBox}>
                    <p style={{ margin: 0, color: '#888' }}>Nenhuma entrada no diário ainda.</p>
                        ...
                        <div key={entrada.id} style={card}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                                <div>
                                    ...
                                    {entrada.atualizado_em && entrada.atualizado_em !== entrada.criado_em && (
                                        <div style={{ fontSize: '11px', color: '#bbb', marginTop: '2px' }}>
                                            Editado em {new Date(entrada.atualizado_em).toLocaleDateString('pt-BR')}
                                        </div>
                                    )}
                                </div>
                                <div style={{ display: 'flex', gap: '4px' }}>
                                    <button onClick={() => setEditando(entrada)} style={btnIcon} title="Editar">
                                        <EditarIcon />
                                    </button>
                                    <button onClick={() => deletar(entrada.id)} style={btnIcon} title="Excluir">
                                        <TrashIcon />
                                    </button>
                                </div>
                            </div>
...
function ModalEntrada({ turmaId, entrada, onClose }) {
    ...
    async function salvar() {
        if (!conteudo.trim()) return setErro('O conteúdo não pode estar vazio');
        setErro(''); setLoading(true);
        try {
            if (editando) {
                await api.put(`/diario/${entrada.id}`, { conteudo });
            } else {
                await api.post('/diario', { turma_id: turmaId, conteudo });
            }
            onClose();
        } catch (e) {
            setErro(e?.message || 'Erro ao salvar entrada');
        } finally {
            setLoading(false);
        }
    }
        ...
        <div style={overlay}>
            <div style={{ ...modal, maxWidth: '520px' }}>
                <h2 style={{ marginBottom: '16px', fontSize: '18px' }}>
                    {editando ? 'Editar entrada' : 'Nova entrada no diário'}
                </h2>

                <textarea
                    placeholder="Descreva o conteúdo da aula, observações, ocorrências..."
                    value={conteudo}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        {erro && <span style={{ fontSize: '12px', color: '#dc2626', alignSelf: 'center' }}>{erro}</span>}
                        <button onClick={onClose} style={btnSec}>Cancelar</button>
                        <button onClick={salvar} style={btnPrimary} disabled={loading}>
                            {loading ? 'Salvando...' : editando ? 'Salvar alterações' : 'Publicar'}
                        </button>
                    </div>
                </div>
            </div>
...
const textarea = { width: '100%', minHeight: '160px', padding: '12px', marginBottom: '14px', border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px', lineHeight: '1.6', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' };
